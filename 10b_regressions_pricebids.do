...
log using regressions_price_log, replace
...
qui reg diff_p_id_da weekend afternoon solar_share wind_share hydro_share load_ACT solar_fc_error wind_fc_error load_fc_error gas_share coal_share co2price gasprice $fe, robust
predict uhat, residuals

qui reg q_total_mg_id weekend afternoon solar_share wind_share hydro_share load_ACT solar_fc_error wind_fc_error load_fc_error gas_share coal_share co2price gasprice $fe, robust
predict uhat2, residuals

qui reg pbid_dummy diff_p_id_da q_total_mg_id solar_share wind_share hydro_share load_ACT solar_fc_error wind_fc_error load_fc_error gas_share coal_share co2price gasprice uhat uhat2 $fe, robust
*-> both uhat's betas are significant

*********Appendix
*****First stage regressions
***(1) IV PF
qui reg diff_p_id_da afternoon weekend solar_share wind_share hydro_share gas_share coal_share load_ACT solar_fc_error wind_fc_error load_fc_error co2price gasprice $fe2, robust 
*outreg2 using 1st_step_firststage.doc, replace ctitle(IV PF Diff) addstat("F-Test", e(F))

qui reg q_total_mg_id afternoon weekend solar_share wind_share hydro_share gas_share coal_share load_ACT solar_fc_error wind_fc_error load_fc_error co2price gasprice $fe2, robust 
*outreg2 using 1st_step_firststage.doc, append ctitle(IV PF Liq) addstat("F-Test", e(F))

***(2) IV Lag
qui reg diff_p_id_da afternoon weekend $lag_controls $fe2, robust
*outreg2 using 1st_step_firststage.doc, append ctitle(IV 1 Lag Diff) addstat("F-Test", e(F))

qui reg q_total_mg_id afternoon weekend $lag_controls $fe2, robust
...
***(1) OLS PF
qui reg pbid_dummy diff_p_id_da q_total_mg_id solar_share wind_share hydro_share gas_share coal_share load_ACT solar_fc_error wind_fc_error load_fc_error co2price gasprice $fe2, robust
cd "$dirpath/Final/RegressionResults"
*outreg2 using 1st_step_robust.doc, replace ctitle(OLS PF) addstat("F-Test", e(F))

****Probit
***(2) Probit PF
qui probit pbid_dummy diff_p_id_da q_total_mg_id solar_share wind_share hydro_share  gas_share coal_share load_ACT solar_fc_error wind_fc_error load_fc_error co2price gasprice $fe2 afternoon weekend
*outreg2 using 1st_step_robust.doc, append ctitle(Probit PF) addstat("Chi2", e(chi2))

****Logit
***(3) Logit PF
qui logit pbid_dummy diff_p_id_da q_total_mg_id solar_share wind_share hydro_share  gas_share coal_share load_ACT solar_fc_error wind_fc_error load_fc_error co2price gasprice $fe2 afternoon weekend
*outreg2 using 1st_step_robust.doc, append ctitle(Logit PF) addstat("Chi2", e(chi2))

*****IV Robustness
***More Lags
***(1) IV 5 Lags
qui ivregress 2sls pbid_dummy (diff_p_id_da q_total_mg_id = afternoon weekend) $lags_controls $fe2, robust
*outreg2 using 1st_step_robust2.doc, replace ctitle(IV 5 Lags) 

***Fewer Controls and FE
**(2) IV PF
qui ivregress 2sls pbid_dummy (diff_p_id_da q_total_mg_id = afternoon weekend) solar_share wind_share hydro_share load_ACT solar_fc_error wind_fc_error load_fc_error co2price gasprice $fe3, robust 
...
**(3) IV PF
qui ivregress 2sls pbid_dummy (diff_p_id_da q_total_mg_id = afternoon weekend) solar_share wind_share hydro_share load_ACT solar_fc_error wind_fc_error load_fc_error co2price gasprice  oil_share gas_share coal_share other_share $fe4, robust 
...
***(1) IV PF
qui ivregress 2sls pbid_dummy (diff_p_id_da_m q_total_mg_id = afternoon weekend) solar_share wind_share hydro_share gas_share coal_share load_ACT solar_fc_error wind_fc_error load_fc_error co2price gasprice $fe2, robust 


****IV Probit Robustness
***More Lags
***(1) IV 5 Lags
qui ivprobit pbid_dummy (diff_p_id_da q_total_mg_id = afternoon weekend) $lags_controls $fe2
*outreg2 using 1st_step_robust3.doc, replace ctitle(IV Probit 5 Lags) addstat("Chi2", e(chi2))

***Fewer Controls and FE
***(3) IV Probit PF
qui ivprobit pbid_dummy (diff_p_id_da q_total_mg_id = afternoon weekend) load_ACT solar_share wind_share hydro_share co2price gasprice wind_fc_error solar_fc_error load_fc_error $fe3
...
**More Controls and FE
***(4) IV Probit PF
qui ivprobit pbid_dummy (diff_p_id_da_m q_total_mg_id = afternoon weekend) load_ACT solar_share wind_share hydro_share co2price gasprice wind_fc_error solar_fc_error load_fc_error co2price gasprice oil_share gas_share coal_share other_share $fe4
...
***OLS
***(1) OLS PF
qui reg pbid_max mg_price_id_max $fe, vce(cluster yq)
*outreg2 using 2ndstep_results.doc, replace ctitle(OLS PF) 

***(2) OLS Lags
qui reg pbid_max mg_price_id_max_lag* $fe, vce(cluster yq)
*outreg2 using 2ndstep_results.doc, append ctitle(OLS 5 Lags) 

**Durwin-Wu-Hausman test 
qui reg mg_price_id_max load_ACT solar_fc_error wind_fc_error load_fc_error co2price gasprice $fe, vce(cluster year)
predict uhat5
qui reg pbid_max mg_price_id_max uhat5 $fe, robust
*-> uhat5 is significant 

***(3) IV PF
qui ivregress 2sls pbid_max (mg_price_id_max=load_ACT solar_share wind_share hydro_share co2price gasprice) $fe, vce(cluster yq)
*outreg2 using 2ndstep_results.doc, append ctitle(IV PF) addstat("Chi²", e(chi2))

***(4) IV Lags
qui ivregress 2sls pbid_max (mg_price_id_max=$instr_lag) $fe, vce(cluster yq)
*outreg2 using 2ndstep_results.doc, append ctitle(IV 1 Lag) addstat("Chi²", e(chi2))

*********Appendix
*****First stage regressions
qui reg mg_price_id_max load_ACT solar_share wind_share hydro_share co2price gasprice $fe, vce(cluster yq)
*outreg2 using 2nd_step_firststage.doc, replace ctitle(IV PF first stage) 

qui reg mg_price_id_max $instr_lag $fe, vce(cluster yq)
*outreg2 using 2nd_step_firststage.doc, append ctitle(IV 1 Lag first stage) 

******Robustness Checks 
***Fewer instruments
***(1) IV PF
qui ivregress 2sls pbid_max (mg_price_id_max=load_ACT wind_share gasprice) $fe, vce(cluster yq)
*outreg2 using 2ndstep_robust.doc, replace ctitle(IV PF small) addstat("Chi²", e(chi2))

***(2) IV Lags
qui ivregress 2sls pbid_max (mg_price_id_max=lag_load_ACT lag_wind_share lag_gasprice) $fe, vce(cluster yq)
*outreg2 using 2ndstep_robust.doc, append ctitle(IV 1 Lag large) addstat("Chi²", e(chi2))

**More instruments
***(3) IV PF
qui ivregress 2sls pbid_max (mg_price_id_max=load_ACT solar_share wind_share hydro_share co2price gasprice load_fc_error solar_fc_error wind_fc_error) $fe, vce(cluster yq)
*outreg2 using 2ndstep_robust.doc, append ctitle(IV PF) addstat("Chi²", e(chi2))

***(4) IV Lags
qui ivregress 2sls pbid_max (mg_price_id_max=lag_load_ACT lag_solar_share lag_wind_share lag_hydro_share lag_co2price lag_gasprice lag_load_fc_error lag_solar_fc_error lag_wind_fc_error) $fe, vce(cluster yq)
...
***(1) OLS PF
qui reg pbid_max mg_price_id_max  $fe5, vce(cluster yq)
cd "$dirpath/Final/RegressionResults"
